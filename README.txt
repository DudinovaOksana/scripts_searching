...
Для работы работы скриптов в системе должен быть установлен Python 3 версии не ниже 3.7
...
ПРИМЕЧАНИЕ:
Для задания 1 надо найти строки вида "CYCLE complete <идентификатор>", содержащие подстроку "Worklist name".
В файле 200731.LOG нет строк  вида "CYCLE complete <идентификатор>", содержащих  подстроку "Worklist name". Там есть строки, содержащие  "CYCLE complete <идентификатор>" и есть отдельно строки  "Worklist name". Соответственно, задачу сформулированную таким образом с приложенным файлом 200731.LOG выполнить невозможно.
В задании 4.1 надо сформировать пару строк -первая вида "info: Slot to run: <ID>" и -следующая, содержащая подстроку вида "info: Slot to run: <ID>". В файле  sc_runtime.log находится только одна строка вида "info: Slot to run: <ID>". Соответственно, в задании 4 невозможно сформировать пары строк требуемым образом.
